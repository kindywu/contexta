...
- **密钥注入（LLM_API_KEY / JWT_SECRET / ADMIN_JWT_SECRET）**：deploy 步骤从 `secrets.*` 以 `KEY='<值>' docker compose up -d` 传入远端（compose `environment` 覆盖 `.env`）；Secret 为空时该键回退 `.env`——本地 `docker compose up` 直跑即后者。更新：`gh secret set <KEY>` → 下一次部署生效。注意：改 `JWT_SECRET` 会使已签发 token 全部失效（需重新登录）。
