...
- **SSE 进度（管理端慢请求）**：重跑/生成类分钟级请求经 SSE 回传进度（progress/done/error + 8s 心跳），`Bun.serve` 保持默认 `idleTimeout` 10s——**不调大治标**（Bun 上限仅 255s，超长仍断；心跳 8s < 10s 保证连接活性）。契约见 architecture.md 管理端端点表。
