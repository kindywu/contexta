...
| `services/auth_service.ts` | 免密直登：自动注册 → 封禁检查 → 会话行 `issued_at` 全局单调（`MAX(issued_at)+1` 与墙钟取大，防时钟回拨，单条 INSERT…SELECT 原子）→ 挤掉保留最新 2 条（`MAX_ACTIVE_DEVICES`）→ token 的 iat 取落库 `issued_at`；**2026-09-18 多设备登录提示扩展**：`login` 记录 `device_name`（机型名，`COALESCE` 不覆盖已有值）、返回 `{token, evicted}`（本次实际被删会话）；`previewEvictions`（纯模拟不落库，与 login 共用 `pickEvicted` 判定规则）；被挤/同设备重登写 `device_evictions` 账本（evicted/relogin 两种 reason，带下手设备快照）；`evictionDetail`（401 EVICTED 的 detail 数据源） |
