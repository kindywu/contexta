...
- **槽位审核视图**（管理端 `GET /api/admin/articles`）：每槽返回 `{slot_id, slot_index, difficulty, status(槽位), attempts, thread_id, article{id, category, title_en, title_zh, source_url, paragraph_count, path, run_date} | null, review{id, status, reject_reason, reviewed_by, reviewed_at} | null, history[]（同槽全部 review 行倒序，含当前行的旧文）}`；status 过滤匹配 review.status，无 review 行槽位按其 slot.status 匹配。
