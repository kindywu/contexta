...
// admin seed → Hono app（/api 路由 + /admin 静态托管 + 顶层 onError）→ Bun.serve →
// 每日任务后台启动 → SIGINT/SIGTERM 优雅退出。
...
export async function main(): Promise<void> {
  ...
  console.log(`[server] listening on :${cfg.port} (db: ${dbPath})`);

  // 6) 每日任务后台启动（startupFill 今天+明天 + 定时循环；失败只记日志）
  ...
  const shutdown = (signal: string) => {
    if (shuttingDown) {
      console.error(`[server] ${signal} 二次信号，强制退出`);
      process.exit(1);
    }
    shuttingDown = true;
    console.log(`[server] ${signal} 收到，优雅退出…`);
    server.stop();
    process.exit(0);
  };
