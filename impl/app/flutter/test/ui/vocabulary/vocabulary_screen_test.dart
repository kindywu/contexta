...
void main() {
  ...
  group('宽屏限宽', () {
    testWidgets('pad 横屏下复习卡限宽居中', (tester) async {
      tester.view.physicalSize = const Size(2438, 1626); // 逻辑 1219×813
      tester.view.devicePixelRatio = 2.0;
      addTearDown(tester.view.resetPhysicalSize);
      addTearDown(tester.view.resetDevicePixelRatio);
      vocabRepo.words = [vocabWord(spelling: 'hello', senses: [sense()])];
      await pumpScreen(tester);

      final card = find.byType(VocabularyCard);
      expect(card, findsOneWidget);
      expect(tester.getSize(card).width, lessThanOrEqualTo(640)); // 限宽列宽
      // 居中：左右留白近似相等
      final left = tester.getTopLeft(card).dx;
      final right = 1219 - tester.getTopRight(card).dx;
      expect((left - right).abs(), lessThan(2));
    });

    testWidgets('手机下复习卡仍占满宽度', (tester) async {
    ...
    testWidgets('pad 横屏下 ✓ FAB 仍在右下角（不受限宽影响）', (tester) async {
