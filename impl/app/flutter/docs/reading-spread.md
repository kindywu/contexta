...
本文描述平板阅读页（`lib/pad/pad_reading_screen.dart`）的**书页模式**：文章被切成一页一页，一次并排显示相邻两页（左页 / 右页），整屏翻页；界面默认只留三样常驻件——左下角的返回键、底部「页码胶囊 + 朗读全文」、以及旁边的译文模式，其余工具栏按需唤出。
...
**2026-09-20 实机对比后的修正**：译文模式原本只在唤出态顶栏里，读者在平板上找不到它——同一台设备上手机顶栏常驻「译文 ▾」，平板却"没有"，于是以为功能缺失（实际在设置页与唤出态顶栏两处都能改）。修法与返回/朗读同一条原则：**高频且需要即时反馈的开关必须常驻**。译文模式并进页码胶囊带右端（成本半枚胶囊，不像返回键要独立圆钮），唤出顶栏时整条带子淡出、由顶栏那枚接替，同一时刻只有一个入口。
...
| 译文模式 | 「译文 完全显示 / 淡化 / 模糊 / 隐藏」常驻在胶囊带右端，点一下循环到下一模式（**不弹菜单**，与手机顶栏 chip 同一套措辞） |
