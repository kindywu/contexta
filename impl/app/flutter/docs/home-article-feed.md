...
3. 服务端已配置且未登录时，最上方多一条「未登录 + 登录入口」横幅（本地模式不显示）。**平板首页有一条同语义、独立实现的登录状态带**（`lib/pad/pad_login_banner.dart`，两棵树不共享 UI）——平板那条尤其关键：它是平板上**唯一**的登录入口，缺了它平板就永远同步不到新文章（见 [adaptive-layout.md](adaptive-layout.md)）。
