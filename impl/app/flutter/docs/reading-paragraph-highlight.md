...
| 场景 | 高亮 | 滚动 |
|------|------|------|
| 全文朗读（KittenTTS）：标题段 | 无（标题不上报） | 不滚动 |
...
- **高亮样式**：仅英文正文文字底色 `Color(0x2ECC785C)`（与生词高亮同色，生词 span 保持原样自然融合）；译文区域不染色；段落内联播放钮图标（volume_up / stop）沿用 `isSpeaking` 语义驱动，全文朗读中当前段的内联钮同样显示 Stop（点击即停止整篇朗读）。
...
全文朗读采用双 worker 流水线：生成 worker（`_generateFullArticle`）推入播放队列，播放 worker（`_playQueued`）顺序消费。原有 `setOnProgress(done/total)` 反映**生成**进度，超前于真实发声，不能作为「正在朗读哪段」的依据。本次在播放 worker 消费到非标题项、`_playWav`/`_playFileSource` 播放**前**上报「段落开始播放」，与真实发声同步。
...
sequenceDiagram
    ...
    Note over PW: 消费到正文段落（paragraphIndex != null）
    PW->>SE: setOnParagraphStarted(utteranceId, paraIndex, total)
    SE->>RC: onParagraphStarted 回调（id 可空收缩非空）
    RC->>RC: utteranceId == _currentUtteranceId && !_disposed 校验
    RC->>RC: state = copyWith(speakingParagraphIndex: paraIndex)
    RC-->>UI: provider 通知
    alt 全文朗读中 (isSpeakingFullArticle)
        UI->>UI: addPostFrameCallback(_scrollToParagraph)
...
| 层 | 文件 | 职责 |
|----|------|------|
| 会话层 | `lib/data/tts/kitten_tts_session.dart` | `_QueuedAudio.paragraphIndex`（正文携带索引，标题为 null）；`_playQueued` 播放前上报；`total` 由 `speakFullArticle` 传入 |
...
| 控制器 | `lib/ui/reading/reading_controller.dart` | `_onTtsReady` 中注册；id 校验过滤迟到旧事件；更新 `speakingParagraphIndex` |
| 视图层 | `lib/ui/reading/reading_screen.dart` | 高亮 span 构造；`ref.listen` 驱动滚动；手滚检测与跳过 |
...
| 播放中停止 / 被新播放打断 | `_stopCurrent()` → finish(oldId) → 控制器清空索引 |
...
| UI 高亮 | `reading_screen_test.dart` | 播放加底色 / 停止消失（按 WidgetSpan 定位段落 RichText） |
| UI 滚动 | `reading_screen_test.dart` | 段 2 对齐 (视口−段高)/3；段 0 clamp 不动；单段不滚动（滚动后内联播段 2，删门控即 RED）；手滚跳过一次下次恢复；未构建段估算滚动 |

已知缺口：`KittenTtsPluginSession`（会话层）不可自动化测试（kit.KittenTTS / AudioPlayer 具体类不可注入），上报时机（发声前、标题跳过、缓存命中路径）由真机验证覆盖。
