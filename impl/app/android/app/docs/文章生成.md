...
启动时 `reconcileOrphanArticles` 把卡在 `GENERATING` 的文章重置回 `PENDING`——由于 claim 是 CAS，即使批次仍被处理，Worker 重跑也不会重复生成。
...
```mermaid
flowchart TD
    A[LlmCaller.call systemPrompt+userPrompt] --> B[withTimeout 120s]
